...
GROUP_ID = db.get_group_id()  # возможно не нужно
...
def get_thread_id(update: Update, context: CallbackContext):
    ...
    if update.message.is_topic_message:
        ...
        context.bot.send_message(
            chat_id=GROUP_ID,
            text=
            f"Ветка {update.message.reply_to_message.forum_topic_created.name} установлена для приветствий."
        )
    else:
        update.message.reply_text(
            "❌ Эта команда работает только внутри ветки форума!")
...
#def ping():
 #   while True:
  #      try:
   #         requests.get(
    #            ""
     #       )  # Запрос к своему проекту
      #      requests.get("https://google.com")  # Дополнительный запрос
       # except Exception as e:
        #    print(f"Ping error: {e}")
        #time.sleep(60)  # Каждые 5 минут
...
def run_bot():
    ...
    dp.add_handler(CommandHandler("role", set_role))
...
if __name__ == '__main__':
    ...
    #   target=app.run,
    #    kwargs={'host': '0.0.0.0', 'port': 8080}  # Используем порт 8080
    #)
    #flask_thread.start()

    # Запуск ping-запросов
